...
export const actions = {
    ...
    async addReguler(context: ActionContext, payload: RegulerCreateRequest) {
        let client = new RegulersServiceClient(baseUrl, {}, {})

        return await client.createOrUpdate(payload, {
            authorization: context.getters.getToken
        }).catch(error => {
            if(error.code && error.code === 16){
                (this as any).$router.push('/')
